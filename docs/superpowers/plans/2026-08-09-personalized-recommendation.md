...
- Create: `src/main/java/cumt/zongzuo/community/recommendation/dto/RecommendationMode.java`
- Create: `src/main/java/cumt/zongzuo/community/recommendation/dto/RecommendationFeatureSnapshot.java`
- Create: `src/main/java/cumt/zongzuo/community/recommendation/dto/RecommendationExposureDraft.java`
- Create: `src/main/java/cumt/zongzuo/community/recommendation/dto/RecommendationSession.java`
- Create: `src/main/java/cumt/zongzuo/community/recommendation/dto/RecommendationSessionItem.java`
...
- Create: `src/main/java/cumt/zongzuo/community/recommendation/service/RecommendationSessionUnavailableException.java`
...
- Modify: `src/main/java/cumt/zongzuo/community/exception/GlobalExceptionHandler.java`
- Modify: `script.sql`
...
- Test: `src/test/java/cumt/zongzuo/community/recommendation/service/RecommendationChronologicalFeaturesTest.java`

**Interfaces:**
- Consumes Task 2 `RecommendationEventOutboxService`, Task 1 properties, Task 4 article-derived feature assembly and the existing chronological feed ordering contract; this task deliberately serves only cold-start chronology until Task 6 adds a validated model.
...
Use JSON value storage under `recommendation:session:{uuid}` with `RecommendationSession(userId, List<RecommendationSessionItem> items, RecommendationMode mode)`, expiry from `sessionTtlMinutes`, and a Base64URL cursor built as `uuid + ":" + offset`. `RecommendationSessionItem` contains `articleId`, `reason`, `source` and an optional immutable `RecommendationFeatureSnapshot`; never store full article bodies or exposure IDs in Redis. The snapshot fixes the nine ordered values used at delivery time: five continuous features and four source one-hot flags. Chronological items may leave it null and build a Redis-independent zero-profile snapshot only when their page is delivered. Personalized items written by Task 6 must provide the exact snapshot used for model scoring; exposure delivery uses it verbatim and never recomputes chronology.

On first page create a `COLD_START` session from up to 100 article IDs selected by a parameterized `SELECT id` query over published, non-deleted rows ordered by `(create_time DESC, id DESC)`. Add the matching `(status, is_deleted, create_time DESC, id DESC)` database index. This is the same chronology semantics as the latest feed but large enough for stable multi-page scrolling. Task 6 replaces the session-construction scoring policy after it has a validated model while reusing the snapshot-aware paging/exposure path. `RecommendationExposure` has `id`, `userId`, `articleId`, `sessionId`, canonical readable `source`, the five continuous feature columns, four `TINYINT` source flags, `exposedAt`, and `createTime`.

Slice the requested page IDs, batch-hydrate only that slice from MySQL while preserving session order, and batch-fill author name/avatar through `UserService#getUserMapCached`; on Redis cache failure, fall back to one `listByIds` database query. Immediately before that page is returned, convert each hydrated item and its stored-or-chronological snapshot to a `RecommendationExposureDraft` and call transactional `RecommendationExposureService.recordPage`. The whole page of exposure rows commits or rolls back together. Its upsert handles only the unique identity conflict, is idempotent for `(user_id, article_id, session_id)`, and returns the existing IDs on a repeated cursor request; other database errors must propagate. Never persist exposure rows for candidates that have not been returned to the user. On later pages, decode with a bounded input length, reject malformed/missing/other-user sessions into chronological fallback, and slice without recomputing rank.
...
`fallback` uses the parameterized chronology mapper with a composite `(create_time, article_id)` cursor so equal timestamps neither duplicate nor skip articles. The cursor is bound to the authenticated user. Every no-cursor fallback request is a fresh real exposure session; only replaying the same valid non-empty fallback cursor deterministically reuses exposure IDs. The only semantic distinction from cold start is `mode=FALLBACK`. A fallback page records only the articles actually returned. If Redis profiles are unavailable, affinities/similarity are zero at serving time while heat/freshness remain article-derived; do not fabricate non-zero profile features. Do not introduce a public security permit rule for `/api/recommendations/**`.
