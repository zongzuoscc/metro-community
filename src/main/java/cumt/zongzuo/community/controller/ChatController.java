...
public class ChatController {
    ...
    @PostMapping("/send")
    public Result<String> send(@RequestParam Long toId, @RequestParam String content) {
    ...
    // 3. 获取聊天记录
    @GetMapping("/history")
    public Result<List<ChatMsg>> getHistory(@RequestParam Long friendId) {
        Long userId = CurrentUser.id();
